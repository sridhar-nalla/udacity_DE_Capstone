...
def plot_missing_values(df, show_label):
    nulls_df = pd.DataFrame(data=df.isnull().sum(), columns=['values'])
    nulls_df = nulls_df.reset_index()
    nulls_df.columns = ['cols', 'values']
    ...
    plt.rcdefaults()
    plt.figure(figsize=(10, 5))
    ax = sns.barplot(x='cols', y='% missing values',
                     data=nulls_df)
    plt.title("% Missing values to exclude")
    ax.set_ylim(0, 100)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=90)
    if (show_label):
...
#     nan_count_df = df.select([count(when(isnan(c) | col(c).isNull(), c)).alias(
...
def plot_missing_values_spark(df, show_label, threshold_value):
    nan_count_df = df.select([count(when(isnan(c) | col(c).isNull(), c)).alias(
        c) for c in df.columns]).toPandas()
    nan_count_df = pd.melt(nan_count_df, var_name='cols', value_name='values')
    total = df.count()
    ...
    if (show_label):
        for c in ax.containers:
            labels = [round(v, 2) if v >
                      threshold else "" for v in c.datavalues]
            ax.bar_label(c, labels=labels, fmt='%.0f', )
    plt.show()
...
def clean_spark_temperature_data(df):
    ...
    print('Total records after dropping rows with missing values: {}'.format(
        total_records-total_recs_after_dropping_nas))

    # drop duplicate rows
    df = df.drop_duplicates(subset=['dt', 'City', 'Country'])
    print('Rows dropped after accounting for duplicates: {}'.format(
        total_recs_after_dropping_nas-df.count()))
...
def aggregate_temperature_data(df):
    ...
    """
    new_df = df.select(['Country', 'AverageTemperature']
                       ).groupby('Country').avg()
...
def clean_spark_demographics_data(df):
    ...
    # drop duplicate columns
    new_df2 = new_df.dropDuplicates(
        subset=['City', 'State', 'State Code', 'Race'])

    rows_dropped_with_duplicates = new_df.count()-new_df2.count()
    print(
        f"Rows dropped after accounting for duplicates: {rows_dropped_with_duplicates}")
