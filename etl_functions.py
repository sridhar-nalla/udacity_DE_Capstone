...
def create_immigration_fact_table(spark, df, output_data):
    ...
    get_datetime = udf(lambda x: (dt.datetime(
        1960, 1, 1).date() + dt.timedelta(x)).isoformat() if x else None)

    # rename columns to align with data model
    # df = df.withColumnRenamed('ccid', 'record_id') \
    df = dim_df.withColumnRenamed('ccid', 'record_id') \
...
def create_visa_type_dimension_table(df, output_data):
    ...
    visatype_df = visatype_df.withColumn(
        'visa_type_key', monotonically_increasing_id())
...
def create_immigration_calendar_dimension(df, output_data):
    ...
    calendar_df = df.select(['arrdate']).withColumn(
        "arrdate", get_datetime(df.arrdate)).distinct()
    ...
    calendar_df = calendar_df.withColumn(
        'arrival_weekday', dayofweek('arrdate'))
    ...
    calendar_df.write.parquet(output_data + "immigration_calendar",
                              partitionBy=partition_columns, mode="overwrite")
...
def quality_checks(df, table_name):
    ...
    if total_count == 0:
        ...
        print(
            f"Data quality check passed for {table_name} with {total_count} records.")
    return 0
