...
def create_spark_session():
   ...
   # process_immigration_data(spark, input_data, output_data,
    #                          immigration_file_name, temperature_file_name, mapping_file)
...
def process_immigration_data(spark, input_data, output_data, file_name, temperature_file, mapping_file):
    ...
    immigration_df = spark.read.format(
        'com.github.saurfang.sas.spark').load(immigration_file)

    # clean immigration spark dataframe
    immigration_df = utility.clean_spark_immigration_data(immigration_df)

    # create visa_type dimension table
    visatype_df = etl_functions.create_visa_type_dimension_table(
        immigration_df, output_data)

    # create calendar dimension table
    calendar_df = etl_functions.create_immigration_calendar_dimension(
        immigration_df, output_data)

    # get global temperatures data
    temp_df = process_global_land_temperatures(
        spark, input_data, temperature_file)

    # create country dimension table
    dim_df = etl_functions.create_country_dimension_table(
        spark, immigration_df, temp_df, output_data, mapping_file)

    # create immigration fact table
    fact_df = etl_functions.create_immigration_fact_table(
        spark, immigration_df, output_data)


def process_demographics_data(spark, input_data, output_data, file_name):
    """Process the demographics data and create the us_demographics_dim table
    ...
    demographics_df = spark.read.csv(
        file, inferSchema=True, header=True, sep=';')

    # clean demographics data
    new_demographics_df = utility.clean_spark_demographics_data(
        demographics_df)

    # create demographic dimension table
    df = etl_functions.create_demographics_dimension_table(
        new_demographics_df, output_data)
...
def main():
    ...
    input_data = "s3a://sridhar.nalla1/"
    output_data = "s3a://sridhar.nalla1/"
    ...
    mapping_file = input_data + "i94res_L.csv"
    ...
    #     spark, input_data, output_data, usa_demographics_file_name)

    df_spark = spark.read.format(
        'com.github.saurfang.sas.spark').load('i94_apr16_sub.sas7bdat')
    df_spark.show(5)
